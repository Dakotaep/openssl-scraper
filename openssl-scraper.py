...
for i in range(len(urls)):
    ...
    # openssl_release = openssl_release and (' 1.0' in url_text or ' 0.' in url_text)
    if openssl_release:
        print(url_text)
        try:
            ...
            if ' 1.' in url_text or ' 0.' in url_text:
                ...
                if ' 1.0' in url_text or ' 0.' in url_text:
                    subprocess.run([full_path + 'config', 'no-shared', 
                                    f'--prefix={build_directory}', f'--openssldir={build_directory}'], cwd=full_path)
                if ' 1.' in url_text:
                    subprocess.run([full_path + 'config', 'no-shared',  'CFLAGS=-g', 
                                    f'--prefix={build_directory}', f'--openssldir={build_directory}'], cwd=full_path)
